...
const int _N=200005; ll inv[_N<<2],fac[_N<<2],fac_inv[_N<<2], _p;
...
inline void init_inv(int n) { inv[1]=1; rep(i,2,n+1) inv[i]=mul(_p-_p/i,inv[_p%i]); }
...
struct Calculator{
	...
	inline void Init_Com_Num_H_B(V a[],int n,ll k) {
		a[0]=1; rep(i,1,n) a[i]=mul(a[i-1],inv[i]*(k-i+1)%_p);
	}
	inline void Init_Com_Num_L_B(V a[],int n,ll k) {
		a[0]=1; rep(i,1,n) a[i]=mul(a[i-1],inv[i]*(k+i)%_p);
	}
